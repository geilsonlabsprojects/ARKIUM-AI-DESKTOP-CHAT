...
pub async fn execute_query(sql: String, params_json: Vec<Value>) -> Result<QueryResult, String> {
    // Safety: only allow safe SQL operations
    let sql_lower = sql.to_lowercase().trim().to_string();
    if sql_lower.starts_with("drop ") || sql_lower.starts_with("truncate ") {
        return Err("DROP and TRUNCATE operations are not allowed via this API".to_string());
    }
